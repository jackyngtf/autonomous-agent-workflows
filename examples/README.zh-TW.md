...
[離線示範](../demo/README.zh-TW.md)才是作品集中可執行的部分。它以合成輸入及本機檔案展示 Avaya 與 NAS 報表的檢查、重跑及阻止有問題的更新。示範加強後的檢查，不代表每個歷史程式都實作了相同行為。eTMS 另有[專屬證據](../docs/evidence/etms-deployment-review-2026-09-22.zh-TW.md)，記錄原始碼檢視、shadow 診斷、修正測試及 9 月 23 日的 19 SWAPPED、8 HELD live 報告。其後檔案與 HTTP 檢查支持本次結果，並清楚列出封存標籤及回復限制。
