...
The [dated evidence](../../docs/evidence/etms-deployment-review-2026-09-22.md) now includes tested repairs, verified deployment and an authorised live report recording 19 SWAPPED and 8 HELD. Read-only follow-up found 19 destinations and archives, 19 HTTP responses matching current NAS targets by hash, and eight held inputs remaining. Stronger backup and recovery claims require captured pre-run hashes and controlled failure/recovery tests. Old archive labels currently use the incoming revision; they do not prove archived content revisions. All eight held cases require document-identity review, and rename suggestions are not approvals. The run trigger and scheduler reliability remain unverified.
