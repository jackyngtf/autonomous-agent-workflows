...
使用者手動觸發的 Cowork 執行記錄 **30 HELD、0 PLANNED、0 SWAPPED**。其後唯讀檢查確認，22 項目標缺漏訊息所指的檔案全部存在於 NAS。離線重現支持本機暫存的診斷。已測試的修正在 2026 年 9 月 22 日完成部署，備份及部署後取回摘要值均已核對；設定維持 shadow，修正後的 Cowork 執行仍待使用者觸發。八項配對個案仍需覆核。較早的執行仍屬暫緩的 shadow 結果，不是成功的正式更新。排程時區、成功的 live 替換及回復仍未驗證。這部分證據與兩個可執行報表示範及 92 份報告清單分開。
