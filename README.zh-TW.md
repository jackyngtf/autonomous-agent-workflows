...
報表案例透過三個實際營運事件說明設計決定，並以可執行的合成資料示範正常更新、重複執行及驗證失敗。eTMS 案例呈現另一種操作邊界：排程 Agent 將檔案修改交給專用 engine。部署檢視與一次手動觸發的 shadow 執行，展示為何設定、程序結束碼及逐份文件結果需要分開核對。
...
使用者手動觸發的 Cowork 執行記錄 **30 HELD、0 PLANNED、0 SWAPPED**。其後唯讀檢查確認，22 項目標缺漏訊息所指的檔案全部存在於 NAS。離線重現支持本機暫存的診斷。已測試的修正在 2026 年 9 月 22 日完成部署，備份及部署後取回摘要值均已核對；設定維持 shadow。使用者 16:35 重跑的 27 項輸入屬較早 30 項的子集，報告記錄 **19 PLANNED、8 HELD、0 SWAPPED**。八項暫緩個案仍需核對文件身份，改名建議不代表批准。較早的執行仍屬暫緩的 shadow 結果。靜態檢視另發現 live 發佈的路徑解析缺口，可能跳過上傳檢查卻仍進入收件區清理；19 項已規劃不代表已可進行 live 更新。9 月 23 日，已測試防護版 driver 完成部署，備份已核對，設定切換至 live；首次 live Cowork 執行仍待確認。排程時區、成功的 live 替換及回復仍未驗證。這部分證據與兩個可執行報表示範及 92 份報告清單分開。
