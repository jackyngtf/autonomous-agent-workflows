...
The [eTMS entry contract](../examples/etms-doc-swap/README.md) specifies a third weekday task at 09:30, with no timezone stated in the inspected file. It hands document updates to a separate engine rather than following the workbook sequence below. The [dated review](evidence/etms-deployment-review-2026-09-22.md) records manual shadow attempts, tested repairs and an operator-authorised switch to live. The 23 September live report records 19 SWAPPED and 8 HELD, supported by read-only file and HTTP checks; its trigger was not independently verified. This single outcome does not establish scheduler reliability.
