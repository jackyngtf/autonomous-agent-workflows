...
The [eTMS case](../case-study/07-etms-document-handoff.md) combines deployment/source inspection, held shadow reports, offline repair checks and a **23 September live report with 19 SWAPPED and 8 HELD**. Read-only follow-up confirmed 19 destinations and archives, 19 served-copy hashes matching current NAS targets, and eight held inputs remaining. Archive revision labels are defective; exact pre-run backup preservation and rollback safety are not established. This evidence contributes no rows to the 92-file reporting ledger.
...
- [eTMS deployment and run review](etms-deployment-review-2026-09-22.md): source findings, shadow diagnosis, tested repairs, verified deployment, and the 23 September live result with read-only checks and archive-label limitations.
