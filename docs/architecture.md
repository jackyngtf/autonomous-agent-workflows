...
The third job, [eTMS document updates](case-study/07-etms-document-handoff.md), delegates file planning and execution to an external driver. Its [dated evidence](evidence/etms-deployment-review-2026-09-22.md) follows a staging failure through tested repairs and an authorised live run reporting 19 SWAPPED and 8 HELD, with subsequent read-only checks. Shadow skips production-document swaps but still performs inbox housekeeping and report uploads; a zero driver exit does not establish engine success. The workbook pipeline below applies only to the reporting jobs.
