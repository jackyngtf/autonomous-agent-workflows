...
The portfolio also documents the [eTMS scheduled handoff](07-etms-document-handoff.md), which assigns file replacements to a separate engine. Entry instructions establish the intended scope and operator-controlled modes. The [dated review](../evidence/etms-deployment-review-2026-09-22.md) traces a held shadow run through source diagnosis, isolated repair tests, verified deployment and a live report with 19 SWAPPED and 8 HELD, supported by read-only file and HTTP checks. It documents integration and troubleshooting work; it does not establish authorship of the original engine or general rollback safety.
