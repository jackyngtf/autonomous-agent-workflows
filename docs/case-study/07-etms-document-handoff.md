...
A subsequent [static finding](../evidence/etms-deployment-review-2026-09-22.md#live-publication-limitation-found-after-the-shadow-run) identified a live-publisher parser that missed paths containing whitespace. It could skip upload checks while still reaching inbox cleanup after local swaps. The guard passed nine offline test methods and was deployed on 23 September with verified backups, before the operator authorised live configuration. The [deployment record](../evidence/etms-deployment-review-2026-09-22.md#guard-deployment-and-live-configuration-23-september-2026) captures the new driver hash.

The **23 September live report records 19 SWAPPED and 8 HELD**. Read-only follow-up confirmed 19 destinations and 19 archives, 19 HTTP-200 responses whose hashes matched the current NAS targets, and exactly eight held inputs remaining. An archive-label defect remains: the engine names old archives using the incoming revision. No pre-run destination hashes were captured, so exact backup preservation and rollback safety are not independently established. The [live-run evidence](../evidence/etms-deployment-review-2026-09-22.md#observed-live-run-23-september-2026) records one publication outcome; the trigger and scheduler reliability remain unverified.
