...
A user-triggered Cowork run reported **30 HELD, 0 PLANNED and 0 SWAPPED**. All 22 targets associated with missing-target messages were then found on the NAS through read-only checks. An offline reproduction supported the local-staging diagnosis. The tested repair was deployed on 22 September 2026 with verified backups and read-back hashes; configuration remained shadow. The user's 16:35 rerun reported **19 PLANNED, 8 HELD and 0 SWAPPED** for 27 inputs, a subset of the earlier 30. All eight held cases still require document-identity review; rename suggestions are not approvals. The earlier run remains a held shadow result. Static review also found a live-publication path-parsing gap that could bypass upload checks while still reaching inbox cleanup; 19 planned items do not establish live readiness. On 23 September, the tested guard driver was deployed and configuration changed to live with verified backups; the first live Cowork run remains pending. Scheduler timezone, successful live swaps and rollback remain unverified. The evidence is separate from the two runnable reporting demos and the 92-report ledger.
