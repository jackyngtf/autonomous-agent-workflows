...
A user-triggered Cowork run reported **30 HELD, 0 PLANNED and 0 SWAPPED**. All 22 targets associated with missing-target messages were then found on the NAS through read-only checks. An offline reproduction supported the local-staging diagnosis. The tested repair was deployed on 22 September 2026 with verified backups and read-back hashes; configuration remained shadow and the user's post-fix Cowork run is pending. Eight matching cases still require review. The earlier run remains a held shadow result, not a successful live update. Scheduler timezone, successful live swaps and rollback remain unverified. The evidence is separate from the two runnable reporting demos and the 92-report ledger.
