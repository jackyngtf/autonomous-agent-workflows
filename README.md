...
The reporting case study follows those decisions through three operational incidents, with runnable synthetic examples of successful updates, repeat runs and validation failure. The eTMS case follows a held shadow run through diagnosis, tested repairs and a live run with 19 reported swaps, checked against stored and served files.
...
The first manual shadow run held all 30 inputs. Read-only checks found all 22 reportedly missing targets on the NAS; an offline reproduction traced the problem to local staging. After repair, a 27-input rerun produced **19 PLANNED and 8 HELD**. A further guard fixed publication of paths containing spaces before the operator authorised live mode.

On **23 September 2026**, the live report recorded **19 SWAPPED and 8 HELD**. Read-only follow-up found all 19 destinations and archives; all 19 HTTP responses matched their current NAS destinations by SHA-256, and only the eight held inputs remained. Those eight require document-identity review. A remaining defect labels old archives with the incoming revision; no pre-run destination hashes were captured to independently prove exact backup preservation. The dated evidence records these limits separately from the reporting demos and 92-report ledger.
